...
const defaultConfig: Config = { apiUrl: 'http://localhost', timeout: 3000 };

const config = await Result.pipe(
  loadConfigFromFile(),
  Result.orElse(() => loadConfigFromEnv()),
  Result.orElse(() => Result.succeed(defaultConfig)),
);
// Tries file first, then env, then falls back to default
```
