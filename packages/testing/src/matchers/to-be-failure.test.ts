...
import { toBeFailure } from './to-be-failure';
...
describe('toBeFailure', () => {
  describe('when received is a Failure', () => {
    ...
    it('should return pass as true', () => {
      const { pass } = toBeFailure.call(state, value);

      expect(pass).toBe(true);
    });

    it('should call callback with the error value', () => {
      const callback = vi.fn();

      toBeFailure.call(state, value, callback);
  ...
  describe('when received is a Success', () => {
    ...
    it('should return a message describing the received value', () => {
      const { message } = toBeFailure.call(state, value);

      expect(message()).toContain('Expected value to be a failure result');
    });
  });
  ...
  describe('when isNot is true', () => {
    ...
    describe.each([
      ...
      it('should return a message suggesting to use toBeSuccess instead', () => {
        const { message } = toBeFailure.call(state, value);

        expect(message()).toBe('Do not use .not with .toBeFailure, please use .toBeSuccess instead.');
      });
    });
  });
});
