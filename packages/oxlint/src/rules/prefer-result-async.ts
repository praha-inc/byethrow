  ...
  createOnce: (context) => {
    return {
      TSTypeReference: (node) => {
        ...
        if (!isByethrowTypeName(innerType, settings.namespace, ['Result'])) return;
        if (!hasTypeArguments(innerType, 2)) return;

        const importSource = getImportSource(context.sourceCode.getScope(node), innerType.typeName.left);
